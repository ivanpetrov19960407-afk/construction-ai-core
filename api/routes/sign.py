...
def _cryptopro_rest_url() -> str:
    return str(getattr(settings, "cryptopro_rest_url", ""))


def _cryptopro_api_key() -> str:
    return str(getattr(settings, "cryptopro_api_key", ""))


def _cryptopro_cert_thumbprint() -> str:
...
async def sign_document_inner(
    ...
    headers = {"Authorization": f"Bearer {_cryptopro_api_key()}"}
    async with httpx.AsyncClient(timeout=60) as client:
        response = await client.post(
            f"{_cryptopro_rest_url().rstrip('/')}/api/v1/sign",
            json=sign_payload,
            headers=headers,
        )
        response.raise_for_status()
...
async def verify_signature(
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        response = await client.post(
            f"{_cryptopro_rest_url().rstrip('/')}/api/v1/verify",
            json=verify_payload,
            headers=headers,
        )
        response.raise_for_status()
