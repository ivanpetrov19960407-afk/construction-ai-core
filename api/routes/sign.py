...
def _upload_related_signed_files(
    ...
    signed_url = str(
        client.generate_presigned_url(
            "get_object",
            Params={"Bucket": settings.s3_bucket_albums, "Key": signed_key},
            ExpiresIn=3600,
        )
    )
    sig_url = str(
        client.generate_presigned_url(
            "get_object",
            Params={"Bucket": settings.s3_bucket_albums, "Key": sig_key},
            ExpiresIn=3600,
        )
    )
    return signed_url, sig_url
...
async def sign_document(payload: SignDocumentRequest, request: Request):
    ...
    signed_url, sig_url = await asyncio.to_thread(
...
async def verify_signature(doc_id: str, request: Request):
    ...
    pdf_url = str(doc.get("pdf_url") or "")
    sig_url = str(doc.get("sig_url") or "")
    if not pdf_url or not sig_url:
