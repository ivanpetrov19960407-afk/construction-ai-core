...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const storedDefaultRole =
        (await store.get<ChatRole>("default_role")) || "pto_engineer";
      setApiUrl(url);
      setApiKey(key);
      ...
      setConnectionStatus({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Не удалось сохранить настройки API.",
      });
    }
  };
  ...
  const onCheckConnection = async () => {
    ...
    try {
      ...
      setConnectionStatus({
        tone: "checking",
        message: "Сервер доступен. Проверяю API Key...",
      });
      ...
      setConnectionStatus({
        tone: "success",
        message:
          "Сервер доступен, API Key принят. Desktop готов к работе с серверным API.",
      });
    } catch (error) {
      setHealth(null);
      setLastCheckedAt(new Date().toISOString());
      setConnectionStatus({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Не удалось проверить соединение с API.",
      });
    }
  };
  ...
  const onLinkTelegram = async () => {
    ...
    try {
      ...
      setLinkStatus(
        "Telegram успешно привязан. Уведомления теперь будут приходить в desktop.",
      );
      setTelegramCode("");
    } catch (error) {
      setLinkStatus(
  ...
  return (
    <Card>
      <form
        onSubmit={onSave}
        style={{ display: "grid", gap: spacing.md, maxWidth: 640 }}
      >
        <p style={{ margin: 0, color: colors.textSecondary }}>
          Текущая роль:{" "}
          {isAdmin ? "Администратор" : (me?.role ?? "не определена")}
        </p>
        <Input
          label="API URL"
          value={apiUrl}
        ...
        <label style={{ display: "grid", gap: spacing.xs }}>
          ...
          <select
            value={defaultRole}
            onChange={(event) =>
              setDefaultRoleValue(event.target.value as ChatRole)
            }
            style={{
        ...
        <div style={{ display: "flex", gap: spacing.sm, flexWrap: "wrap" }}>
          ...
          >
            {connectionStatus.tone === "checking"
              ? "Проверка..."
        ...
        {linkStatus && (
          <p style={{ margin: 0, color: colors.textSecondary }}>{linkStatus}</p>
        )}
        ...
        {documentsCount === 0 && (
          ...
          >
            ...
            >
              ...
              <li>
                Нажмите «Загрузить PDF» и выберите файл с нормативами (СП,
                ГОСТ).
              </li>
              <li>
                Дождитесь сообщения об успешной загрузке и вернитесь в чат.
              </li>
            </ol>
            <Button
              type="button"
      ...
      {health && (
        <Card padding="md" style={{ marginTop: spacing.lg }}>
          <h3 style={{ marginTop: 0, marginBottom: spacing.sm }}>
            Статус сервера
          </h3>
          <p style={{ marginTop: 0 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {Object.entries(health.components).map(([key, component]) => {
                const statusIcon =
                  component.status === "ok" || component.status === "active"
                    ? "✅"
                    : "⚠️";
                const details =
                  key === "rag_engine"
          ...
          {(health.components.rag_engine?.sources as number | undefined) ===
            0 && (
      ...
      <Card padding="md" style={{ marginTop: spacing.lg }}>
        ...
        <p style={{ margin: 0 }}>
          Версия сервера: {health?.version || "не определена"}
        </p>
        <p style={{ margin: 0 }}>
          Дата последней проверки соединения:{" "}
          {lastCheckedAt
            ? new Date(lastCheckedAt).toLocaleString("ru-RU")
            : "ещё не выполнялась"}
        </p>
      </Card>
    </Card>
