...
export default function AnalyzeTenderPage() {
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      setRequirements(Array.isArray(payload.requirements) ? payload.requirements.map(String) : []);
      setRisks(Array.isArray(payload.risks) ? payload.risks.map(String) : []);
      setDeadlines(Array.isArray(payload.deadlines) ? payload.deadlines.map(String) : []);
      setEstimate(String(payload.estimated_budget ?? payload.estimate ?? ''));
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : 'Ошибка анализа тендера.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Card>
      ...
      <p style={{ marginTop: 0, color: colors.textSecondary }}>Главная / Анализ / Тендер</p>
      <div
