...
export default function GenerateKSPage() {
  ...
  const normalizeKSResponse = (
    ...
    const header: KSHeader = {
      object_name:
        normalizedKs2Raw.object_name ||
        normalizedKs3Raw.object_name ||
        payloadHeader.object_name,
      contract_number:
        normalizedKs2Raw.contract_number ||
        normalizedKs3Raw.contract_number ||
        payloadHeader.contract_number,
      period_from:
        normalizedKs2Raw.period_from ||
        normalizedKs3Raw.period_from ||
        payloadHeader.period_from,
      period_to:
        normalizedKs2Raw.period_to ||
        normalizedKs3Raw.period_to ||
        payloadHeader.period_to,
    };

    const workItems = Array.isArray(normalizedKs2Raw.work_items)
      ? normalizedKs2Raw.work_items
      : [];
    const ks2: KS2Data = {
      ...header,
      work_items: workItems,
      total_cost: Number(
        normalizedKs2Raw.total_cost ?? response.total_cost ?? 0,
      ),
      total_hours: Number(
        normalizedKs2Raw.total_hours ?? response.total_hours ?? 0,
      ),
    };
    const ks3: KS3Data = {
      ...header,
      ...
      total_cost: Number(
        normalizedKs3Raw.total_cost ?? response.total_cost ?? 0,
      ),
      total_hours: Number(
        normalizedKs3Raw.total_hours ?? response.total_hours ?? 0,
      ),
    ...
    <Card>
      <section style={{ display: "grid", gap: spacing.md }}>
        ...
        <form
          ...
          style={{ display: "grid", gap: spacing.md, maxWidth: 900 }}
        >
          <Input
          ...
          <div style={{ display: "grid", gap: spacing.sm }}>
                  ...
                  {workRows.map((row, rowIndex) => (
                    <tr key={row.id}>
                      ...
                      >
                        <Input
                          error={
                            validationErrors[`work_items.${rowIndex}.name`]
                          }
                          value={row.name}
                          onChange={(event) =>
                      ...
                      {(["volume", "normHours", "pricePerUnit"] as const).map(
                        (field) => (
                          <td
                            key={`${row.id}-${field}`}
                            style={{
            ...
            {importInfo && (
              <p style={{ color: colors.textSecondary }}>{importInfo}</p>
            )}
        ...
        {summary && (
          <p style={{ color: colors.textPrimary, fontWeight: 600 }}>
            ...
            {summary.total_cost || 0} руб. | Всего нормо-часов:{" "}
