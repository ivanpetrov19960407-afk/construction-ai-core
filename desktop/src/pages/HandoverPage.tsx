...
function normalizeForecast(
  payload: Partial<ScheduleForecast>,
): ScheduleForecast {
  return {
...
export default function HandoverPage() {
  ...
  useEffect(() => {
    const savedProjectId =
      window.localStorage.getItem("handover_project_id") ?? "";
  ...
  const loadSigning = async (): Promise<ProjectInfo> => {
    ...
    setDocuments(
      (docsResponse.documents || []).filter(
        (doc) => (doc.status ?? "approved") === "approved",
      ),
    );
  ...
  const handleSign = async (doc: ProjectDocument) => {
    ...
    try {
      ...
      setError(
        signError instanceof Error ? signError.message : "Ошибка подписи",
      );
    }
  };
  ...
  const handleBatchSign = async () => {
    ...
    if (!signablePendingDocuments.length) {
      setBatchProgress({ signed: 0, total: 0 });
      setError(
        "Нет документов поддерживаемых типов для пакетной подписи (aosr, ks2, ks3)",
      );
      return;
    }

    const docsByType = signablePendingDocuments.reduce<
          ...
          try {
            ...
            const message =
              batchError instanceof Error
                ? batchError.message
                : "Ошибка пакетной подписи";
            batchErrors.push(`${docType}: ${message}`);
          }
          setBatchProgress({
            signed: signedTotal,
            total: signablePendingDocuments.length,
          });
        }
      }
      if (newlySignedIds.size > 0) {
        setSignedDocIds((prev) => [
          ...new Set([...prev, ...Array.from(newlySignedIds)]),
        ]);
      }
            ...
            <datalist id="my-projects">
              {myProjects.map((item) => (
                <option
                  key={item.id}
                  value={item.id}
                >{`#${item.short_id} — ${item.name}`}</option>
              ))}
            </datalist>
          </label>
          <label style={{ display: "grid", gap: 4 }}>
            ...
            },
            {
              ...
              content: (
                <section style={{ display: "grid", gap: 10 }}>
                        ...
                        >
                          ...
                          {project?.is_state_contract &&
                            status === "signed" && (
                              <Button
                                type="button"
                                onClick={() => handleIsupSubmit(doc.id)}
                              >
                                Передать в ИСУП
                              </Button>
                            )}
                  ...
                  {!documents.length && (
                    <p>Нет документов со статусом «утверждён».</p>
                  )}
                  ...
                  {project?.is_state_contract && (
                    <section style={{ display: "grid", gap: 8 }}>
                      ...
                      {isupSubmissions.length > 0 ? (
                        ...
                        <p style={{ margin: 0 }}>
                          Отправки в ИСУП пока отсутствуют.
                        </p>
                      )}
                    </section>
                  )}
                ...
                <section style={{ display: "grid", gap: 12 }}>
                  ...
                  >
                    ...
                    <p style={{ margin: "8px 0" }}>
                      Экспорт акта выполненных работ в формат 1С
                    </p>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <Input
                ...
                <section style={{ display: "grid", gap: 10 }}>
                  ...
                  >
                    ...
                    <p style={{ marginTop: 0 }}>
                      ...
                      {((forecast?.delay_rate ?? 0) * 100).toFixed(1)}%
                    </p>
                  </article>
                  {allRiskProjects.length > 0 && (
                    ...
                    >
                      <strong>
                        🔴 Высокий риск задержки: {allRiskProjects.length}{" "}
                        проект(ов)
                      </strong>
                      <ul>
                        {allRiskProjects.slice(0, 5).map((projectRisk) => (
                          <li key={projectRisk.project_id}>
                            {projectRisk.project_name} —{" "}
                            {Math.round(projectRisk.delay_rate * 100)}%
                            вероятность, прогноз:{" "}
                            {projectRisk.predicted_completion}
                          </li>
                        ))}
                      </ul>
                    </article>
